...
export default function Step2Config({
  ...
  currentOrder: Order;
  setCurrentOrder: (updates: any) => void;
  sizeOptions: Array<{ label: string; value: string }>;
  mailClassOptions: Array<{ label: string; value: string }>;
}) {
  const prices = usePublicStore((s) => s.prices);
  const templates = useTemplateStore((s) => s.templates);
  const selectedTemplate =
    templates.find((t) => t._id === currentOrder.templateId) || null;
  ...
  useEffect(() => {
    ...
    const existing = Array.isArray(currentOrder.globalDesignVariables)
      ? [...currentOrder.globalDesignVariables]
      : [];
    let changed = false;
  ...
  const effectiveSizeOptions =
    prices && prices.length > 0
      ? Array.from(
          new Map(
            prices.map((p) => [
              p.sizeKey + "|" + p.sizeLabel,
              { label: p.sizeLabel, value: p.sizeKey },
            ])
          ).values()
        )
      : sizeOptions;
            ...
            {designFields.map((f: any) => {
              ...
              return (
                <Component
                  ...
                  name={f.fieldKey}
                  value={
                    (currentOrder.globalDesignVariables || []).find(
                      (v: any) => v.key === f.fieldKey
                    )?.value || ""
                  }
                  onChange={(value: any) =>
                    handleDesignFieldChange(f.fieldKey, value)
                  }
      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Select
            ...
            disabled={!!currentOrder.designId}
