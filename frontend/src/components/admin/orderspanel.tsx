    ...
    return (
        <Card className="hover:shadow-lg transition-shadow p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                <div>
                    <p><strong>Mail Class:</strong> {order.mailClass}</p>
                    <p><strong>Mail Date:</strong> {order.mailDate}</p>
                    <p><strong>Recipients:</strong> {order.recipients?.length || 0}</p>
                    {order.pcmOrderId && (
                        <p><strong>PCM Order ID:</strong> {order.pcmOrderId}</p>
                    )}
                </div>
                <div>
                    <p><strong>User Email:</strong> {order.userDet?.email || "N/A"}</p>
                    <p><strong>User Phone:</strong> {order.userDet?.phone || "N/A"}</p>
            ...
            {order.returnAddress && (
                <div className="mt-3 text-sm text-gray-700">
                    ...
                    <p>
                        ...
                        {order.returnAddress.address2 ? `, ${order.returnAddress.address2}` : ""}
                    </p>
                    <p>
            ...
            {/* Conditional Details */}
            {isLetter ? <LetterDetails order={order} /> : <PostcardDetails order={order} />}
                ...
                {front || back ? (
                    <div className="flex flex-col md:flex-row gap-4 justify-center">
                        {front && (
                            <Button
                                onClick={() =>
                                    showProofModal(front, productType === "letter" ? "Letter Proof" : "Front Proof")
                                }
